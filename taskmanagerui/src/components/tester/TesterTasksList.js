...
class TesterTasksList extends Component {
    constructor(props) {
        super(props);
        this.state = {
          searchValue: "",
      ...
      handleViewChange(e,taskid,maintaskid) {
        localStorage.setItem("maintaskId", maintaskid)
      ...
      handleProgressChange(e,taskid){
        localStorage.setItem("taskId",taskid)
        window.location.href = "/tester-add-progress";
      }
      ...
      async handleOnLoad() {
        await fetch('http://localhost:8080/task/developer/all?userId='+sessionStorage.getItem("userId"), {
          method: 'GET',
          headers: {
            "Accept": "application/json",
            "Content-Type": "application/json"
                      ...
                      {this.state.taskData && this.state.taskData.length > 0 && this.state.taskData.filter(task => new Date(task.duedate) >= new Date()).map((task, index) => {
                          return (
                            <tr className="odd" key={index}>
                              ...
                              <td>{task.duedate}</td>
                              <td>
                                <a href="#" onClick={evt => this.handleViewChange(evt, task.taskid,task.maintaskid)} className="grn_table_link">View All tasks  </a>
                                <a href="#" onClick={evt => this.handleProgressChange(evt, task.taskid)} className="grn_table_link">Add Progress  </a>
                              </td>
                            </tr>);
...
export default TesterTasksList;